...
## 优先级
- VLIW 打包：同一周期内填满各引擎可用槽位，减少指令包数量。
- 向量化内层循环：按 `VLEN`（=8）批处理连续元素，使用 `vload/vstore/valu/vbroadcast/vselect`。
- 地址生成优化：基址外提，`flow.add_imm` 增量推进；减少重复加法与取模。
- 分支消除：用 `vselect` 实现 `+1/+2` 选择与越界回绕判断，避免控制流分岔。
- 调试最小化：性能测试时关闭或减少 `debug` 槽以免污染周期。
